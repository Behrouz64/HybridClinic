...
class DynamicDataManager:
    def __init__(self):
        ...
        # پیدا کردن مسیر امن دایرکتوری در اندروید و ویندوز
...
def main(page: ft.Page):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if data.get("status") is False:
                    status_bar.value, status_bar.color = f"❌ {data.get('message', 'محدودیت ارسال')}\n💡 راهنما: شماره دیگری را امتحان کنید.", "orange"
                else:
                    status_bar.value, status_bar.color = "✅ کد تایید با موفقیت پیامک شد.", "green"
            else:
                status_bar.value, status_bar.color = "❌ سرور درخواست را رد کرد.\n💡 راهنما: آی‌پي بلاک شده، حالت پرواز ✈️ بزنید.", "red"
        except:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if "token" in data:
                    ...
                    sms_text_field.value = db.load_settings().get("sms_text", "از مراجعه شما سپاسگزاریم.")
                    preview_container.visible = True
                    status_bar.value, self.status_bar.color = "✅ کد تایید شد. گزینه نهایی را انتخاب کنید.", "green"
                else:
                    status_bar.value, status_bar.color = f"❌ {data.get('message', 'کد منقضی شده')}\n💡 راهنما: مجدداً دکمه دریافت کد را بزنید.", "red"
            else:
    ...
    def do_end_shift():
        ...
        # ساخت فایل CSV خروجی بله
        output = io.StringIO()
        output.write('﻿')
        writer = csv.writer(output)
        writer.writerow(["ردیف", "موبایل", "درمان", "نظر"])
        for i, log in enumerate(logs, 1):
            writer.writerow([i, log.get("mobile"), log.get("category"), log.get("comment", "").replace("
", " ")])
            
        try:
            res_bale = requests.post(
                "https://tapi.bale.ai/bot1137791878:xD-QEx6ZHEuqnzBmBFRUklgzo7wFqTDrOmY/sendDocument",
                data={"chat_id": "4448378011", "caption": f"📊 گزارش پایان شیفت کلینیک\n👤 منشی: {state['user']}\n👥 تعداد: {len(logs)}"},
