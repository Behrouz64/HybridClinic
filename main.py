...
def main(page: ft.Page):
    ...
    # -----------------------------------------------------------------
    # دوقلوهای پایش پس‌زمینه شبکه (ترد غیرمسدودکننده و کاملاً مجاز اندروید)
    # -----------------------------------------------------------------
    def sync_with_server_bg():
        try:
            ...
            if res.status_code == 200:
                ...
                login_err.value = "✅ دیتابیس آنلاین کلینیک با موفقیت همگام‌سازی شد."
                login_err.color = ft.Colors.GREEN
                page.update()
        except:
            login_err.value = "⚠️ حالت آفلاین؛ لیست پیش‌فرض کاربران لود شد."
    ...
    # اجرای امن تردها در لایه پس‌زمینه بدون مسدود کردن فلاتر
