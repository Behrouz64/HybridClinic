...
# =====================================================================
# ۱. مدیریت پایگاه داده (معماری شیء‌گرا برای جلوگیری از خطای سراسری)
# =====================================================================
class DataManager:
    def __init__(self):
        # 🟢 آدرس قطعی و ۱۰۰٪ مجاز پایتون در اندروید (بدون نیاز به هیچ مجوزی)
        self.base_dir = os.environ.get("HOME", os.getcwd())
        
        self.settings_file = os.path.join(self.base_dir, "settings.json")
        self.comments_file = os.path.join(self.base_dir, "comments.json")
        self.users_file = os.path.join(self.base_dir, "users.json")
        self.logs_file = os.path.join(self.base_dir, "logs.json")
    ...
    def init_db(self):
        ...
        if not os.path.exists(self.settings_file):
            self.save_json(self.settings_file, {"sms_mode": "local", "sms_text": "از مراجعه شما به مطب سپاسگزاریم."})
            
        if not os.path.exists(self.comments_file):
            self.save_json(self.comments_file, {
    ...
    def save_json(self, filepath, data):
        try:
            ...
            raise Exception(f"خطا در نوشتن فایل {filepath}: {e}")

    def load_settings(self): return self.load_json(self.settings_file, {})
    def save_settings(self, data): self.save_json(self.settings_file, data)
    def load_comments(self): return self.load_json(self.comments_file, {})
    def load_users(self): return self.load_json(self.users_file, {})
    def load_logs(self): return self.load_json(self.logs_file, [])
    def save_logs(self, data): self.save_json(self.logs_file, data)
    def clear_logs(self): self.save_json(self.logs_file, [])
...
class SettingsView(ft.View):
    def __init__(self, page: ft.Page, db: DataManager):
        ...
        self.status_text = ft.Text(f"دیتابیس در مسیر امن گوشی فعال است.", color=ft.Colors.BLUE, size=12)
...
def main(page: ft.Page):
    try:
        # لود گرافیک قبل از هر کار دیگری تا سفیدی از بین برود
        page.title = "سامانه کلینیک"
        page.theme_mode = ft.ThemeMode.LIGHT
        page.rtl = True
        page.auth_user = None
        
        loading_text = ft.Text("⏳ در حال خواندن حافظه گوشی...", size=16, color=ft.Colors.BLUE)
        ...
        # اتصال به دیتابیس بومی به صورت ایزوله
        db = DataManager()
        db.init_db()
        ...
        page.scroll = "auto"
        page.add(
            ft.Text("🚨 خطای دسترسی به حافظه اندروید", color="white", bgcolor="red", weight="bold", size=18),
            ft.Text("علت: اندروید شما اجازه ساخت فایل در این مسیر را نمی‌دهد.", color="black", weight="bold"),
