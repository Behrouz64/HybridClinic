...
def get_safe_data_dir():
    # الگوریتم پیدا کردن پوشه ۱۰۰٪ مجاز در اندروید برای جلوگیری از ارور Permission
    dirs_to_try = [
        os.getenv("FLET_APP_STORAGE_DATA"),
        os.path.join(os.getenv("HOME", ""), "hybrid_data") if os.getenv("HOME") else None,
        os.path.join(os.getcwd(), "hybrid_data"),
        os.path.join(tempfile.gettempdir(), "hybrid_data")
    ]
...
def initialize_database():
    global SETTINGS_FILE, COMMENTS_FILE, USERS_FILE, LOGS_FILE, GLOBAL_DATA_DIR
    
    base_dir = get_safe_data_dir()
    GLOBAL_DATA_DIR = base_dir
...
def load_settings(): return load_json(SETTINGS_FILE, {"sms_mode": "local", "sms_text": "از مراجعه شما به مطب سپاسگزاریم."})
def save_settings(data): save_json(SETTINGS_FILE, data)
def load_comments(): return load_json(COMMENTS_FILE, {})
def save_comments(data): save_json(COMMENTS_FILE, data)
def load_users(): return load_json(USERS_FILE, {})
def save_users(data): save_json(USERS_FILE, data)
def load_logs(): return load_json(LOGS_FILE, [])
def save_logs(data): save_json(LOGS_FILE, data)
def clear_logs(): save_json(LOGS_FILE, [])
...
class SettingsView(ft.View):
    def __init__(self, page: ft.Page):
        ...
        self.status_text = ft.Text(f"مسیر دیتابیس: {GLOBAL_DATA_DIR}", color=ft.Colors.BLUE, size=10)
...
class HomeView(ft.View):
    ...
    def check_auth_live(self):
        if not self.current_user or not load_users().get(self.current_user, {}).get("is_active"):
    ...
    def on_verify(self, e):
        ...
        try:
            ...
            if res.status_code == 200 and "token" in res.json():
                ...
                self.preview_field.value = random.choice(load_comments().get(self.app_state["category"], ["عالی است"]))
    ...
    def on_randomize(self, e):
        opts = load_comments().get(self.app_state.get("category"), ["عالی است"])
...
# =====================================================================
# ۵. بخش روتینگ با سیستم ضد کرش و لاگر خطای گرافیکی
# =====================================================================
def main(page: ft.Page):
    try:
        # ساختن دیتابیس در همان ابتدای کار با الگوریتم امن
        initialize_database()
        ...
        # 🔴 لاگر خطای گرافیکی قدرتمند: جلوگیری کامل از صفحه سفید
        page.clean()
        page.scroll = "auto"
        page.add(
            ft.Text("🚨 خطای بحرانی در پایتون (صفحه سفید متوقف شد)", color="white", bgcolor="red", weight="bold", size=18),
            ft.Text("لطفاً از متن زیر اسکرین‌شات بگیرید:", color="red"),
