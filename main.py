...
# =====================================================================
# روی اندروید، بهترین محل برای فایل‌های محلی، همان پوشه‌ای است که برنامه از آن اجرا می‌شود.
# این مسیر معمولاً داخل فضای اختصاصی اپ است و از PermissionError جلوگیری می‌کند.
APP_DATA_DIR = os.getenv("FLET_APP_STORAGE_DATA") or os.path.dirname(os.path.abspath(__file__)) or os.getcwd()
...
DEFAULT_SETTINGS = {"sms_mode": "local", "sms_text": "از مراجعه شما به مطب سپاسگزاریم."}
DEFAULT_COMMENTS = {
...
def load_json(filepath, default_val):
    ...
    # اگر فایل نبود یا خراب بود، دوباره با مقدار پیش‌فرض بساز
    try:
...
def initialize_files():
    ...
    load_json(COMMENTS_FILE, DEFAULT_COMMENTS)
    load_json(USERS_FILE, DEFAULT_USERS)
    load_json(LOGS_FILE, DEFAULT_LOGS)
...
def load_settings():
    ...
    if "sms_mode" not in data:
        data["sms_mode"] = "local"
    if "sms_text" not in data:
        data["sms_text"] = DEFAULT_SETTINGS["sms_text"]
    return data
...
def load_comments():
    data = load_json(COMMENTS_FILE, DEFAULT_COMMENTS)
    if not isinstance(data, dict):
        data = DEFAULT_COMMENTS.copy()
...
def load_users():
    ...
    # اطمینان از وجود admin در هر حال
    if "admin" not in data:
        data["admin"] = DEFAULT_USERS["admin"].copy()
        save_json(USERS_FILE, data)
    return data
...
def load_logs():
    data = load_json(LOGS_FILE, DEFAULT_LOGS)
...
def send_sms(page, mobile, text, mode="local"):
    ...
    if not mobile or len(mobile) != 11 or not mobile.startswith("09"):
        try:
            page.snack_bar = ft.SnackBar(ft.Text("❌ خطای فرمت شماره موبایل."))
    ...
    if mode == "local":
        try:
            ...
            logging.warning(f"SMS local launch failed: {e}")
            return False
        ...
        if not api_key or not sender:
            try:
                ...
                logging.warning(f"SMS fallback launch failed: {e}")
                return False
        ...
        try:
            ...
            logging.warning(f"SMS api launch failed: {e}")
            try:
                encoded_text = urllib.parse.quote(text)
                page.launch_url(f"sms:{mobile}?body={encoded_text}")
                return False
...
def main(page: ft.Page):
    ...
    # نمایش خطاها به صورت قابل‌دیدن روی دستگاه برای جلوگیری از صفحه سفید
    def on_error(e):
        try:
            msg = getattr(e, "data", None) or str(e)
            logging.error(f"Flet page error: {msg}")
            page.snack_bar = ft.SnackBar(ft.Text(f"❌ خطا: {msg}"))
            page.snack_bar.open = True
            page.update()
        except Exception:
            logging.exception("Unhandled page error")
    ...
    def route_change(route):
        try:
            ...
            logging.exception("Route change failed")
            page.views.clear()
            page.views.append(
                ft.View(
                    route="/",
                    controls=[
                        ft.Container(
                            content=ft.Column(
                                [
                                    ft.Text("خطا در بارگذاری برنامه", size=22, weight="bold"),
                                    ft.Text(str(e), color=ft.Colors.RED),
                                    ft.Text("لطفاً لاگ خطا را بررسی کنید.")
                                ],
                                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                            ),
                            padding=20,
