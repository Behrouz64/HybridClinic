...
def main(page: ft.Page):
    # پالت رنگی و تم بومی ترموکس شما
    ...
    # -----------------------------------------------------------------
    # همگام‌سازی مستقیم و آنی از سرور مرکزی (دقیقاً در بدو ورود مفسر)
    # -----------------------------------------------------------------
    try:
        res = requests.get("https://api.iranlasik.ir/api/sync", headers={"x-api-token": "Secure_Key_2026"}, timeout=4)
    ...
    def do_login(e):
        ...
        if u in users and users[u]["password"] == p and users[u]["is_active"]:
            ...
            # نمایش دکمه‌های ابزار هدر بعد از ورود منشی
            btn_settings.visible = True
            btn_logout.visible = True
            
            # استارت موتور گوش به زنگ بله پس از لاگین منشی
    ...
    main_panel = ft.Column([
        ...
        code_field,
        category_buttons,
        ft.TextButton("🧹 پاکسازی فرم جاری", on_click=reset_form_click, icon=ft.Icons.REFRESH, icon_color=ft.Colors.RED),
        preview_container,
    ...
    # -----------------------------------------------------------------
    # دوقلوهای پایش پس‌زمینه بله (بدون درگیری با فایل‌های قفل‌شده اندروید)
    # -----------------------------------------------------------------
    def bale_polling_bg():
